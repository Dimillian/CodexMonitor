...
pub(crate) fn resolve_workspace_claude_args(
    entry: &WorkspaceEntry,
    parent_entry: Option<&WorkspaceEntry>,
    app_settings: Option<&AppSettings>,
) -> Option<String> {
    if let Some(value) = entry.settings.claude_args.as_deref() {
        if let Some(normalized) = normalize_claude_args(value) {
            return Some(normalized);
        }
    }
    if entry.kind.is_worktree() {
        if let Some(parent) = parent_entry {
            if let Some(value) = parent.settings.claude_args.as_deref() {
                if let Some(normalized) = normalize_claude_args(value) {
                    return Some(normalized);
                }
            }
    ...
    if let Some(settings) = app_settings {
        if let Some(value) = settings.claude_args.as_deref() {
            return normalize_claude_args(value);
        }
    }
    None
}

fn normalize_claude_args(value: &str) -> Option<String> {
...
#[cfg(test)]
mod tests {
    use super::{parse_claude_args, resolve_workspace_claude_args};
    ...
    fn parses_simple_args() {
        let args =
            parse_claude_args(Some("--profile personal --flag")).expect("parse args");
    ...
    fn parses_quoted_args() {
        let args =
            parse_claude_args(Some("--path \"a b\" --name='c d'")).expect("parse args");
        assert_eq!(args, vec!["--path", "a b", "--name=c d"]);
    }

    #[test]
    fn resolves_workspace_claude_args_precedence() {
        let mut app_settings = AppSettings::default();
        app_settings.claude_args = Some("--profile app".to_string());
        ...
        let parent = WorkspaceEntry {
            ...
            settings: WorkspaceSettings {
                claude_args: Some("--profile parent".to_string()),
        ...
        let resolved = resolve_workspace_claude_args(&child, Some(&parent), Some(&app_settings));
        assert_eq!(resolved.as_deref(), Some("--profile parent"));

        let mut override_child = child.clone();
        override_child.settings.claude_args = Some("  --profile child  ".to_string());
        let resolved_child =
            resolve_workspace_claude_args(&override_child, Some(&parent), Some(&app_settings));
        ...
        let resolved_main = resolve_workspace_claude_args(&main, None, Some(&app_settings));
