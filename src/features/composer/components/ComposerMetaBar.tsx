      ...
      <div className="composer-meta">
          ...
          canUsePlanToggle ? (
            <div className="composer-select-wrap composer-plan-toggle-wrap">
              <label
                ...
                aria-label={t("uiText.composerBar.planMode")}
              >
                <input
                ...
                <span className="composer-plan-toggle-label">
                  {planMode?.label || t("uiText.composerBar.plan")}
                </span>
              </label>
            </div>
          ) : (
            ...
            <span className="composer-icon" aria-hidden>
              ...
              <select
                ...
                aria-label={t("uiText.composerBar.collaborationMode")}
        ...
        <div className="composer-select-wrap composer-select-wrap--model">
          ...
          <select
            ...
            aria-label={t("uiText.composerBar.model")}
          ...
          >
            {models.length === 0 && (
              <option value="">{t("uiText.composerBar.noModels")}</option>
            )}
            {models.map((model) => (
          ...
          {selectedServiceTier === "fast" && (
            <span
              ...
              role="status"
              aria-label={t("uiText.composerBar.fastModeEnabled")}
              title={t("uiText.composerBar.fastModeEnabled")}
            >
        ...
        <div className="composer-select-wrap composer-select-wrap--effort">
          ...
          <select
            ...
            aria-label={t("uiText.composerBar.thinkingMode")}
            value={selectedEffort ?? ""}
            onChange={(event) => onSelectEffort(event.target.value)}
            disabled={disabled || !reasoningSupported}
          >
            {reasoningOptions.length === 0 && (
              <option value="">{t("uiText.composerBar.default")}</option>
            )}
        ...
        {codexArgsOptions.length > 1 && onSelectCodexArgsOverride && (
          <div className="composer-select-wrap">
            ...
            <select
              ...
              aria-label={t("uiText.composerBar.codexArgsProfile")}
              disabled={disabled}
        ...
        <div className="composer-select-wrap">
          ...
          <select
            className="composer-select composer-select--approval"
            aria-label={t("uiText.composerBar.agentAccess")}
            disabled={disabled}
            value={accessMode}
            onChange={(event) =>
