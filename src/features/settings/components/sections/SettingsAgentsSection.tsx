  ...
  return (
    ...
    >
      <div className="settings-help settings-agents-builtins-help">
        {t("settings.agents.builtInRoles")}
      </div>

      <SettingsToggleRow
        title={t("settings.agents.configFile")}
        subtitle={t("settings.agents.configFileSubtitle", {
      ...
      >
        <div className="settings-agents-actions">
          ...
          <button
            ...
            disabled={!settings}
          >
            {t("settings.agents.openInFileManager", { manager: fileManagerName() })}
          </button>
        </div>
      </SettingsToggleRow>

      <SettingsToggleRow
        title={t("settings.agents.enableMultiAgent")}
        subtitle={t("settings.agents.enableMultiAgentSubtitle")}
      >
        <SettingsToggleSwitch
      ...
      <SettingsToggleRow
        ...
        subtitle={t("settings.agents.maxThreadsSubtitle")}
      >
        <div
          className="settings-agents-stepper"
          role="group"
          aria-label={t("settings.agents.maxThreadsAria")}
        >
          <button
            type="button"
            ...
            aria-label={t("settings.agents.decreaseMaxThreads")}
          >
            ▼
          </button>
          ...
          <button
            ...
            aria-label={t("settings.agents.increaseMaxThreads")}
          >
            ▲
          </button>
        </div>
      ...
      <SettingsToggleRow
        title={t("settings.agents.maxDepth")}
        subtitle={t("settings.agents.maxDepthSubtitle")}
      >
        <div
          className="settings-agents-stepper"
          role="group"
          aria-label={t("settings.agents.maxDepthAria")}
        >
          <button
            type="button"
            className="ghost settings-agents-stepper-button"
            ...
            aria-label={t("settings.agents.decreaseMaxDepth")}
          >
            ▼
          </button>
          <div className="settings-agents-stepper-value" aria-live="polite" aria-atomic="true">
          ...
          <button
            ...
            aria-label={t("settings.agents.increaseMaxDepth")}
          >
            ▲
          </button>
        </div>
      ...
      <SettingsSubsection
        title={t("settings.agents.createAgent")}
        subtitle={t("settings.agents.createAgentSubtitle")}
      />
      <div className="settings-field settings-agents-form">
        <div className="settings-agents-description-row">
          ...
          <button
            ...
            title={t("settings.agents.generateWithAi")}
            aria-label={t("settings.agents.generateFieldsForNewAgent")}
          >
            {createDescriptionGenerating ? (
        ...
        <label className="settings-label" htmlFor="settings-agent-create-developer-instructions">
          {t("settings.agents.developerInstructions")}
        </label>
        <textarea
          ...
          placeholder={t("settings.agents.developerInstructionsPlaceholder")}
          disabled={creatingAgent}
        />
        <div className="settings-agents-model-row">
          <div className="settings-agents-model-field settings-agents-model-field--model">
            <span className="settings-agents-inline-label">{t("settings.agents.modelInline")}</span>
            <select
          ...
          <div className="settings-agents-model-field settings-agents-model-field--effort">
            <span className="settings-agents-inline-label">
              {t("settings.agents.reasoningInline")}
            </span>
            <select
              ...
              aria-label={t("settings.agents.agentReasoningEffort")}
            >
              {createReasoningOptions.length === 0 && (
                <option value="">{t("settings.agents.notSupported")}</option>
              )}
        ...
        <div className="settings-agents-actions">
          <button type="button" className="ghost" onClick={() => void handleCreateAgent()}>
            {creatingAgent ? t("settings.agents.creating") : t("settings.agents.createAgent")}
          </button>
        </div>
        {modelOptions.length === 0 && (
          <div className="settings-help">
            {modelOptionsLoading
              ? t("settings.agents.loadingModelMetadata")
              : t("settings.agents.usingFallbackModels")}
          </div>
        )}
      ...
      <SettingsSubsection
        title={t("settings.agents.configuredAgents")}
        subtitle={t("settings.agents.configuredAgentsSubtitle")}
      />

      {settings && settings.agents.length === 0 && !isLoading && (
        <div className="settings-help">{t("settings.agents.noCustomAgents")}</div>
      )}
        ...
        return (
          <div className="settings-field settings-agent-card" key={agent.name}>
            ...
            <div className="settings-agents-actions">
              ...
              >
                {isReadingConfig ? t("settings.agents.opening") : t("settings.agents.editFile")}
              </button>
              {!agent.managedByApp && (
                <span className="settings-help settings-help-inline">
                  {t("settings.agents.externalPath")}
                </span>
              )}
            </div>
            {isPendingDelete && (
              <div className="settings-agents-actions">
                <span className="settings-help settings-help-inline">
                  {t("settings.agents.deleteAgentPrompt")}
                </span>
                <button
                  type="button"
                  className="ghost"
                ...
                <button
                  ...
                  disabled={isDeleting}
                >
                  {isDeleting ? t("settings.agents.deleting") : t("settings.agents.confirmDelete")}
                </button>
              </div>
            )}
              ...
              <div className="settings-agents-edit-form">
                <div className="settings-agents-description-row">
                  ...
                  <button
                    ...
                    title={t("settings.agents.generateWithAi")}
                    aria-label={t("settings.agents.generateFieldsForAgent", {
                      name: agent.name,
                    })}
                  >
                    {editDescriptionGenerating ? (
                ...
                <label className="settings-checkbox">
                  ...
                  />
                   {t("settings.agents.renameManagedConfig")}
                </label>
                <div className="settings-agents-actions">
                  <button
                    type="button"
                  ...
                  <button
                    ...
                    disabled={isUpdating}
                  >
                     {isUpdating ? t("settings.agents.saving") : t("settings.agents.save")}
                    </button>
                </div>
                ...
                <div className="settings-agents-header">
                  <div>
                    <div className="settings-toggle-title">
                      {t("settings.agents.agentConfigFile", { name: agent.name })}
                    </div>
                    <div className="settings-toggle-subtitle">
                  ...
                  <div className="settings-agents-actions">
                    ...
                    >
                      {isWritingConfig ? t("settings.agents.saving") : t("settings.agents.save")}
                    </button>
                  </div>
                </div>
                <textarea
