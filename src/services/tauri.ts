import { invoke as invokeInTauri } from "@tauri-apps/api/core";
...
const MISSING_TAURI_INVOKE_ERROR =
...
async function invokeCommand<T>(command: string, params?: Record<string, unknown>): Promise<T> {
  try {
    return await invokeInTauri<T>(command, params);
  } catch (error) {
    if (isMissingTauriInvokeError(error)) {
      throw new Error(MISSING_TAURI_INVOKE_ERROR);
    }
    throw error;
  }
}
...
async function invokeCommandWithFallback<T>(
  ...
  try {
    ...
    if (error instanceof Error && error.message === MISSING_TAURI_INVOKE_ERROR) {
      console.warn(
        "Tauri invoke bridge unavailable; returning a fallback value for",
        command,
      );
      return fallback;
    }
...
export async function listWorkspaces(): Promise<WorkspaceInfo[]> {
  return invokeCommandWithFallback("list_workspaces", undefined, []);
}
...
export async function addWorkspace(
  ...
  return invokeCommand<WorkspaceInfo>("add_workspace", { path, codex_bin });
}
...
export async function addWorktree(
  ...
  return invokeCommand<WorkspaceInfo>("add_worktree", { parentId, branch, name, copyAgentsMd });
}
...
export async function updateWorkspaceCodexBin(
  ...
  return invokeCommand<WorkspaceInfo>("update_workspace_codex_bin", { id, codex_bin });
}
...
export async function initGitRepo(
  ...
  return invokeCommand<InitGitRepoResponse>("init_git_repo", { workspaceId, branch, force });
}
...
export async function listThreads(
  ...
  return invokeCommand<any>("list_threads", { workspaceId, cursor, limit, sortKey, cwd });
}
...
export async function generateCommitMessage(
  ...
  return invokeCommand("generate_commit_message", { workspaceId, commitMessageModelId });
}
